...
def read_plate(yolo_license_plate, im):
    ...
    # Avoid pandas overhead for speed (fallback to pandas only if needed)
    bb_list = []
    try:
        det = results.xyxy[0]
        if det is not None and len(det):
            ...
            if names is None:
                names = getattr(yolo_license_plate, "names", {})
            ...
            for x1, y1, x2, y2, conf, cls in det_np:
                ...
                if isinstance(names, dict):
                    ...
                    try:
                        ...
                        label = str(cls_int)
                bb_list.append([float(x1), float(y1), float(x2), float(y2), float(conf), cls_int, label])
    except Exception:
        bb_list = []
